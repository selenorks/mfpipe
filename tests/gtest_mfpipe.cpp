...
TEST_P(MFPipeTest, MultiThreading)
{
  std::string m_address = std::get<0>(GetParam());

  MFPipeImpl server;
  ASSERT_EQ(server.PipeCreate(m_address, ""), S_OK);
  MFPipeImpl client;

  int nMaxBuffers = 32;
  ASSERT_EQ(client.PipeOpen(m_address, nMaxBuffers, ""), S_OK);

  const int PACKETS_COUNT = 32;
  std::shared_ptr<MF_BASE_TYPE> arrBuffersIn[PACKETS_COUNT];
  int message_count = 2;
  std::string channels[] = { "ch1", "ch2", "ch4" };
  std::string strEventNames[] = { "event1", "event2" };
  std::string strEventParams[] = { "param1", "event3" };
  ...
  for (int i = 0; i < thread_count; i++) {
    workers.push_back(std::make_unique<std::thread>([&]() {
      for (int j = 0; j < MAX_ITER; j++) {
        int pos = j % message_count;
        const auto& channel = channels[pos];
        const auto& strEventName = strEventNames[pos];
        const auto& strEventParam = strEventParams[pos];
        server.PipeMessagePut(channel, strEventName, strEventParam, PIPE_WAIT_TIMEOUT);
      }
    }));
  }
  ...
  for (int i = 0; i < thread_count; i++) {
    workers.push_back(std::make_unique<std::thread>([&]() {
      for (int j = 0; j < MAX_ITER; j++) {
        ...
        std::string strEventName;
        std::string strEventParam;
        client.PipeMessageGet(channel, &strEventName, &strEventParam, PIPE_WAIT_TIMEOUT);
        //        received.push_back({channel, strEventName, strEventParam});
      }
    }));
  }
  ...
  //        const auto& strEventName = strEventNames[pos];
  //        const auto& strEventParam = strEventParams[pos];
  //        TestMessage test_message{channel,strEventName, strEventParam};
  //        auto iter = std::find(received.begin(), received.end(), test_message);
